...
def load_ui_context(file_path: str) -> Dict[str, Any]:
...
def load_use_cases(file_path: str) -> List[Dict[str, Any]]:
    with open(file_path, "r", encoding="utf-8") as f:
        data = json.load(f)
...
def build_prompt(use_case: Dict[str, Any], base_url: str, ui_context: Dict[str, Any]) -> str:
    use_case_json = json.dumps(use_case, indent=2, ensure_ascii=False)
    ui_context_json = json.dumps(ui_context, indent=2, ensure_ascii=False)
...
- Prefer getByTestId() whenever a matching test id is available.
- Only use selectors that are explicitly provided in the UI context when possible.
- Do NOT invent selectors, labels, or UI elements that are not supported by the use case or UI context.
...
- If the UI text is uncertain, infer carefully from the use case title, steps, and UI context.
...
- Prefer `page.getByTestId('<id>')` over text-based selectors whenever test ids are available.
- Return only the final TypeScript code.

Context:
This app is an Open Pioneer Trails based weather WebGIS application.
Typical features include:
- a map
- a toc / layer switcher
- basemap switching
- a legend
- weather forecast panel
- clicking the map to inspect a location
- measurement tool
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Use provided data-testid selectors whenever available. "
...
def save_test_file(output_dir: str, use_case: Dict[str, Any], ts_code: str) -> Path:
    Path(output_dir).mkdir(parents=True, exist_ok=True)

    file_name = f"uc-{int(use_case['id']):02d}-{slugify(use_case['title'])}.spec.ts"
...
def generate_tests(
    ...
    for use_case in use_cases:
        try:
            ...
            ts_code = extract_typescript_code(response_text)
            validate_generated_code(ts_code)

            file_path = save_test_file(output_dir, use_case, ts_code)
            print(f"Saved: {file_path}")

        except Exception as exc:
            print(f"Failed to generate UC {use_case.get('id')}: {exc}")


def main() -> None:
    use_cases = load_use_cases(USE_CASES_FILE)
    ui_context = load_ui_context(UI_CONTEXT_FILE)
    generate_tests(use_cases, OUTPUT_DIR, BASE_URL, ui_context)
